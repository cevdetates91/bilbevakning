...
MINUS_SIGNALER = [
    ...
    ("servostyrningen har problem", "Servostyrningsproblem nämnt"),
    ("servostyrningen läcker", "Servostyrningen läcker"),
    ("tung styrning", "Tung styrning nämnd (kan tyda på servostyrningsfel)"),
    ("styrningen kärvar", "Styrningen kärvar enligt annonsen"),
...
def _traff_utan_negation(kombinerad: str, nyckel: str, positiva_suffix: tuple[str, ...] = ()) -> bool:
    """
    ...
    OCH utan att direkt följas av en positiv sammansättning (t.ex. "rost" i
    "rostskyddsbehandlad" eller "anmärkning" i "anmärkningsfri" ska INTE räknas).
    """
    ...
    while True:
        ...
        efterfoljande = kombinerad[idx + len(nyckel): idx + len(nyckel) + 15]
        har_negation = any(neg in foregaende for neg in negationer)
        har_positiv_sammansattning = any(efterfoljande.startswith(suf) for suf in positiva_suffix)
        if not har_negation and not har_positiv_sammansattning:
            return True
        start = idx + len(nyckel)


# Ord där sammansatta positiva former (t.ex. "rostSKYDDAD", "anmärkningSFRI")
# annars skulle räknas som falska träffar. nyckel -> tuple av farliga suffix.
_POSITIVA_SUFFIX = {
    "rost": ("fri", "skydd", "skyddsbehandl", "beständig"),
    "anmärkning": ("sfri", "fri"),
}
...
def analysera_djupare(annons: Annons) -> dict:
    ...
    plus = [text for nyckel, text in PLUS_SIGNALER
            if _traff_utan_negation(kombinerad, nyckel, _POSITIVA_SUFFIX.get(nyckel, ()))]
    minus = [text for nyckel, text in MINUS_SIGNALER
             if _traff_utan_negation(kombinerad, nyckel, _POSITIVA_SUFFIX.get(nyckel, ()))]
    info_lista = [text for nyckel, text in SMA_ANMARKNINGAR
                  if _traff_utan_negation(kombinerad, nyckel, _POSITIVA_SUFFIX.get(nyckel, ()))]
